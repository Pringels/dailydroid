...
export default {
  ...
  methods: {
    ...
    getSlackMemberList() {
      this.loading = true;
      return axios.get('http://165.227.161.43/api/v1/user-list').then(({data}) => {
    ...
    getUserList() {
      this.loading = true;
      return axios.get('http://165.227.161.43/api/v1/user').then(({data}) => {
        this.userData = data;
    ...
    registerUser(user) {
      this.loading = true;
      axios.post('http://165.227.161.43/api/v1/user-register', {user}).then(() => {
        this.getUserList();
      })
    },
    removeUser(user) {
      ...
      axios.delete(`http://165.227.161.43/api/v1/user/${userId}`).then(() => {
        this.getUserList();
      })
    }
  }
